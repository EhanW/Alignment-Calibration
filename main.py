...
def get_args():
    ...
    parser.add_argument('--project', '-p', type=str, default='conul-reg-test')
...
def make_dataloaders():
    ...
    train_extractor_transforms_1 = get_cl_augs(strength=args.strength_1, size=args.size, imagenet=imagenet)
    if args.strength_2 is not None:
        train_extractor_transforms_2 = get_cl_augs(strength=args.strength_2, size=args.size, imagenet=imagenet)
    else:
        train_extractor_transforms_2 = None
    ...
    train_unlearn_dataset = DatasetTriad(root=args.data, pair=False, triad=True, train=True, transform=train_extractor_transforms_1, 
                                        transform_2=train_extractor_transforms_2, selected_indices=unlearn_indices, index_shift=args.index_shift)
...
def make_emia_features(model):
    ...
    evaluation_membership_indices = retain_indices[args.num_test_samples:args.num_test_samples+args.num_unlearn_samples] ### membership samples
